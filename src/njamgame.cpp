...
//		if (!SetupNetwork())	// game canceled or network error
    ...
    do
    {
        ...
        SDLKey key;					 			// space to continue, enter to exit to main menu
        do
        {
...
//			break;
        ...
        char screenshot[20];
        sprintf(screenshot, "sc/%03d.bmp", counter/20);
        if (counter % 20 == 0)
...
//        while (StartTime + 34 > SDL_GetTicks() && go) ;				// target about 30 fps
		int delay = 33 - (SDL_GetTicks() - StartTime);
...
void NjamEngine::RenderGameScreen()
{
   int off = (800 - MAPW*25) / 2;
    ...
    for (int i=0; i<4; i++)
    {
        ...
        {
            SDL_Rect dest, src;
            NjamSetRect(dest, off + s->x*25 + s->xo*5, s->y*25 + s->yo*5, 25, 25);
    ...
    for (int i=0; i<4; i++)					
    {
        ...
        {										// so we draw a countdown: 3, 2, 1, ...
            sprintf(buff, "%d", p->Juice / 20);
            m_FontBlue->WriteTextXY(m_Screen,
	            					off+ 25 * m_Sprite[i].x + 5 * m_Sprite[i].xo - 10,
									25 * m_Sprite[i].y + 5 * m_Sprite[i].yo - 10, buff);
        }
    }

	SDL_BlitSurface(m_Background, NULL, m_Screen2, NULL);
    ...
    for (int i=0; i<4; i++)					// render points & (lives | victories)
    {
        ...
        if (p->Playing)
        {
	    	...
	    	if (Duel_game)
	    	{
                sprintf(buff, "%d", p->GamePoints);
	            m_FontBlue->WriteTextXY(m_Screen2, 20 + 25 * i, 25, buff);
	     	}
            ...
            Uint32 FillColor = SDL_MapRGB(0, ply_color[i*3], ply_color[i*3+1], ply_color[i*3+2]);
            SDL_Rect re;
            NjamSetRect(re, 30 + i*25, 50, 20, p->MapPoints);
            SDL_FillRect(m_Screen2, &re, FillColor);

			// draw thin vertical gray bar (how much points would player have if he takes all that's left)
            if (Duel_game && maxp != p->MapPoints)
            {
	            FillColor = SDL_MapRGB(9, 150, 180, 200);

                if (p->MapPoints + m_CookiesLeft < maxp)
                {
		            FillColor = SDL_MapRGB(0, 255, 255, 255);
					Dinger++;
					if (!m_TripleDinged)
					{
                ...
                NjamSetRect(re, 35+i*25, 51+p->MapPoints, 4, m_CookiesLeft);
	            SDL_FillRect(m_Screen2, &re, FillColor);
            }
        }
    }
...
//	SDL_FillRect(m_Screen2, &dest, 0);
	...
	if (m_Bonus)
	{
        ...
        NjamSetRect(dest, 100, 585 - m_Bonus/10 - minus, 22, m_Bonus/10 + 1);
        SDL_FillRect(m_Screen2, &dest, 0);

		NjamSetRect(src, 0, 125 + 209 - m_Bonus/10, 20, m_Bonus/10);
		NjamSetRect(dest, 100, 585 - m_Bonus/10 - minus);
        SDL_BlitSurface(m_SpriteImage, &src, m_Screen2, &dest);
	}
    ...
    if (m_GameType == gtOnePlayer || m_GameType == gtTwoPlayer)
    {
        sprintf(buff, "LEVEL: %02d", m_CurrentMap + 1);
		m_FontYellow->WriteTextXY(m_Screen2, 300, 175, buff);
	}

	if (m_GameType == gtOnePlayer)
	{
        sprintf(buff, "PTS: %4d", m_Player[0].GamePoints);
		m_FontYellow->WriteTextXY(m_Screen2, 300, 275, buff);
	}

    sprintf(buff, "%2.0f FPS", m_fps);
	m_FontYellow->WriteTextXY(m_Screen2, 300, 375, buff);
...
void NjamEngine::RenderSprites()
{
    ...
    for (int i=0; i<GHOSTMAX+4; i++)
    {
        ...
        if (s->delay > MAXDELAY/2 && s->delay%2 && i > 3 && (gh.x != s->x || gh.y != s->y))		// dead ghost?
        {
	        ...
	        NjamSetRect(dest, off + s->x * 25 + s->xo * 5, s->y * 25 + s->yo * 5);
...
//			SDL_SetAlpha(m_SpriteImage, SDL_SRCALPHA, 150 + 105 * top/MAXDELAY);
			NjamSetRect(dest, off + s->x * 25 + s->xo*5 - 1, s->y*25 + s->yo*5 - 2);
			NjamSetRect(src, 35, 50, 28, 31);
			SDL_BlitSurface(m_SpriteImage, &src, m_Screen, &dest);
//			SDL_SetAlpha(m_SpriteImage, SDL_SRCALPHA, 255);
		}
...
//    while ((key = NjamGetch(false)) != SDLK_LAST)
//    {
		key = NjamGetch(false);
        if (key & SDLK_ESCAPE)
            return false;
        ...
        if (key & SDLK_p && m_GameType != gtJoinDuel)		// only server can pause the game
        {
			...
			NjamSetRect(r, 260, 260, 250, 81);
			Uint32 yello = SDL_MapRGB(0, 255, 255, 200);
			SDL_FillRect(m_Screen, &r, yello);
			NjamSetRect(r, 270, 270, 230, 60);
			Uint32 black = SDL_MapRGB(0, 0, 0, 0);
			SDL_FillRect(m_Screen, &r, black);

	        m_FontYellow->WriteTextXY(m_Screen, 310, 280, "PAUSED");
	        SDL_Flip(m_Screen);

            while (NjamGetch(false) & SDLK_p);		// wait to release the key

            while (!(SDLK_p & NjamGetch(false)));  // wait to press the key again

            while (NjamGetch(false) & SDLK_p);		// wait to release the key again to not catch it at next cycle
        }
...
bool NjamEngine::RenderLifeLostScreen(bool LifeAwarded)
{
	...
	SDL_Rect r, src;
	NjamSetRect(r, 240, 260, 290, 81);
 	Uint32 black = SDL_MapRGB(0, 0, 0, 0);
    SDL_FillRect(m_Screen, &r, black);
    NjamSetRect(r, 250, 270, 270, 60);
 	Uint32 green = SDL_MapRGB(0, 25, 155, 25);
    SDL_FillRect(m_Screen, &r, green);

	if (LifeAwarded)
		m_FontYellow->WriteTextXY(m_Screen, 260, 280, "BONUS LIFE");
	else
		m_FontYellow->WriteTextXY(m_Screen, 260, 280, "LIVES LEFT");

	NjamSetRect(src, 75, 75, 25, 25);
	for (int i=0; i<m_LivesLeft; i++)
	{
		if (m_LivesLeft > 8)
			NjamSetRect(r, 240 + i * 10, 350);
		else
			NjamSetRect(r, 249 + i * 25, 350);

        SDL_BlitSurface(m_SpriteImage, &src, m_Screen, &r);
    }

	if (!LifeAwarded)
	    m_FontBlue->WriteTextXY(m_Screen, 130, 450, "PRESS A TO CONTINUE, START TO QUIT");
...
//	while (NjamGetch(false) != SDLK_LAST);		// empty keyboard buffer
...
void NjamEngine::ReadySetGo()
{
    ...
    for (int i=0; i<3; i++)
    {
        ...
        m_FontYellow->WriteTextXY(m_Screen, 330 + (i == 2 ? 5 : 0), 280, message[i]);
		SDL_BlitSurface(m_Background, NULL, m_Screen2, NULL);
        SDL_Flip(m_Screen);
        SDL_Delay(1000);
    }
...
{
 	SDL_Rect src, dest;
 	NjamSetRect(dest, 82, 60);
	SDL_BlitSurface(m_StatsImage, NULL, m_Screen2, &dest);
    ...
    for (int i=0; i<4; i++)
    {
		...
		NjamSetRect(dest, 130, 220 + 30*i);
		SDL_BlitSurface(m_SpriteImage, &src, m_Screen2, &dest);

		if (m_GameType == gtOnePlayer || m_GameType == gtTwoPlayer)
	        sprintf(points, "  (%3d)", m_Player[i].MapPoints);
		else
	        sprintf(points, "%01d (%3d)", m_Player[i].GamePoints, m_Player[i].MapPoints);
        m_FontBlue->WriteTextXY(m_Screen2, 135, 225 + 30*i, points);

        for (int j=0; j<6; j++)
        {
            sprintf(points, "%4d", m_Player[i].stats.data[j]);
            m_FontBlue->WriteTextXY(m_Screen2, 245 + 74*j, 225 + 30*i, points);
        }
    }

	if (m_GameType == gtOnePlayer)
	{
		sprintf(points, "POINTS: %d", m_Player[0].GamePoints);
		m_FontBlue->WriteTextXY(m_Screen2, 140, 260, points);
	}
}
//-----------------------------------------------------------------------------
...
//		TopTenScores[i+1].name[0] = '\0';
		TopTenScores[i+1].name[0] = '3';
		TopTenScores[i+1].name[1] = 'D';
		TopTenScores[i+1].name[2] = 'S';
		TopTenScores[i+1].name[3] = '\0';
/*
		// ENTER YOUR NAME
		...
		while (true)
		{
			...
			if ((key == SDLK_ENTER) && TopTenScores[i+1].name[0])	// entered
				break;
		}
*/
		m_LastHiscore = i+1;
